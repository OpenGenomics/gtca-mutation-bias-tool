...
for line in maf:
    if line.startswith("#") or line.startswith("Hugo"):
        ...
        this_key = '|'.join([Tumor_Sample_Barcode, Chromosome, Start_Position, End_Position, Reference_Allele, Tumor_Seq_Allele2])
        if (Reference_Allele=="C" and Tumor_Seq_Allele2=="A"):
            if Tumor_Sample_Barcode in ca_dict2:
                ca_dict2[Tumor_Sample_Barcode] += 1
            else:
                ca_dict2[Tumor_Sample_Barcode] = 1
        if (Reference_Allele=="G" and Tumor_Seq_Allele2=="T" and Tumor_Sample_Barcode[::-1].startswith("90")):
            if Tumor_Sample_Barcode in filter_dict:
                filter_dict[Tumor_Sample_Barcode][0] += 1
            else:
                filter_dict[Tumor_Sample_Barcode] = [1,0,vaf_dict[Tumor_Sample_Barcode]] #number of variants, number filtered, min_vaf
            this_vaf = int(line[41])/int(line[39])
...
filterstats.write('\t'.join(["TCGA_ID","Number_of_G>T_variants","Number_of_G>T_variants_filtered","Minimum_VAF_for_inclusion","Number_of_C>A_variants"])+"\n")
for k,v in filter_dict.items():
    filterstats.write(k+"\t"+'\t'.join([str(x) for x in v])+"\t"+str(ca_dict2[k])+"\n")
